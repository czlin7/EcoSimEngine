...
class ComponentManager
{
    struct IStore
    {
        ...
        virtual void erase(std::size_t id) noexcept = 0;
    };
    ...
    struct Store final : IStore
    {
        ...
        void erase(std::size_t id) noexcept override
        {
            data.erase(id);
        }
    };
...
public:
    ...
    void entityDestroyed(std::size_t id) noexcept
    {
        for (auto &[_, store] : m_stores)
        {
            store->erase(id);
        }
    }
