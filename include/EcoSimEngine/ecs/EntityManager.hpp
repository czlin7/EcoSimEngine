...
class EntityManager
{
    ...
    // Reference to the SystemManager and ComponentManager is required for proper ECS operation.
...
public:
    ...
    T &addComponent(const std::shared_ptr<Entity> &entity, Args &&...args)
    {
        ...
        m_systemManager.EntitySignatureChanged(entity->id(), entity->signature()); // notify systemManager of signature change
        return comp;
    }
    ...
    void destroyEntity(const std::shared_ptr<Entity> &entity)
    {
        ...
        m_systemManager.EntityDestroyed(entity->id());
    }
